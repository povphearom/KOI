package com.phearom.koi.api.core.adapter;
...
import com.phearom.koi.api.core.binder.ItemBinder;
import com.phearom.koi.api.core.listener.BindViewHandler;
import com.phearom.koi.api.core.listener.ClickHandler;
import com.phearom.koi.api.core.listener.EndLessHandler;
import com.phearom.koi.api.core.listener.LongClickHandler;
import com.phearom.koi.api.core.listener.TopLessHandler;
...
public class BindingRecyclerViewAdapter<T> extends RecyclerView.Adapter<BindingRecyclerViewAdapter.ViewHolder> implements View.OnClickListener, View.OnLongClickListener {
    ...
    private BindViewHandler<T> bindViewHandler;
    private TopLessHandler topLessHandler;
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        if (null != bindViewHandler)
            bindViewHandler.onBindView(item, position);

        if (null != topLessHandler)
            if (position < topLessHandler.getOffset() && !topLessHandler.isLoading()) {
                topLessHandler.setLoading(true);
                topLessHandler.onLoad(position);
            }

        if (null != endLessHandler)
            if (position > items.size() - endLessHandler.getOffset() && !endLessHandler.isLoading()) {
                endLessHandler.setLoading(true);
                endLessHandler.onLoad(position);
            }
    ...
    public void setBindViewHandler(BindViewHandler<T> bindViewHandler) {
        this.bindViewHandler = bindViewHandler;
    }

    public void setTopLessHandler(TopLessHandler topLessHandler) {
        this.topLessHandler = topLessHandler;
    }

    public void setEndLessHandler(EndLessHandler endLessHandler) {
        this.endLessHandler = endLessHandler;
    }
